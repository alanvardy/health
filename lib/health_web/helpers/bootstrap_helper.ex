defmodule HealthWeb.BootstrapHelper do
  ...
  def tab(label, opts \\ [], [do: _] = block) do
    ...
    nav_opts = [
      class: "nav-link", id: "tab-#{id}-#{controls}", role: "tab", href: "##{id}-#{controls}",
      "data-toggle": "tab", "aria-controls": "#{id}-#{controls}", "aria-selected": false
    ]

    div_opts = [
      class: "tab-pane fade", id: "#{id}-#{controls}", role: "tabpanel",
